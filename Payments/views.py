...
stripe.api_key =  os.environ.get('pk_test_51KK0SgSA95CTv5cXVWVGdtVY027sODRSr2iJ9LOvQqFbTdP7go7xUuU94CVFsI8155sjvKHvO9o31TxGPnbdh8IL00v13JyyPO')
...
def booking(request, id):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            return redirect('checkout.html', id=form_item.id)
    else:
        form = BookingForm()
    context = {'form':form}
...
def checkout(request, id):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            form_item.save()
            messages.success(request,f"Payment for booking of INR {amount} has been initiated")
    size = Booking.objects.all().values()
...
def confirmation_mail(request, id):
    if request.user.is_authenticated:
        ...
        <body>
            ...
            <p> Thank you for purchasing a Travel Package from Tourizto! Your Payment for Booking {name} has been received.</p>
            <br>
            <table>
                <tr>
                    <th>Destination</th>
        ...
        sender_address = ('uniquemanjunath00@gmail.com')
        sender_pass = ('lcrlynpebuyoecgu')
